% PARAMUX(5) Version @@VERSION@@ | paramux terminal configuration file

# NAME

**paramux** - paramux terminal configuration file

# DESCRIPTION

To configure paramux, you must use a configuration file. GUI-based configuration
is on the roadmap but not yet supported. The configuration file must be placed
at `$XDG_CONFIG_HOME/paramux/config.ghostty`, which defaults to `~/.config/paramux/config.ghostty`
...
`$HOME/Library/Application Support/io.github.soldforaloss.paramux/config.ghostty`.** This is the
...
If your configuration file has any errors, paramux does its best to ignore
...
macOS and Linux (GTK). paramux does not treat configuration errors as fatal and
will fall back to default values for erroneous keys.

You can also view the full configuration paramux is loading using `paramux
+show-config` from the command-line. Use the `--help` flag to additional options
for that command.

## Logging

paramux can write logs to a number of destinations. On all platforms, logging to
`stderr` is available. Depending on the platform and how paramux was launched,
logs sent to `stderr` may be stored by the system and made available for later
retrieval.

On Linux if paramux is launched by the default `systemd` user service, you can use
`journald` to see logs: `journalctl --user --unit app-io.github.soldforaloss.paramux.service`.

On macOS logging to the macOS unified log is available and enabled by default.
--Use the system `log` CLI to view paramux logs: `sudo log stream --level debug
--predicate 'subsystem=="io.github.soldforaloss.paramux"'`.

paramux logging can be configured in two ways. The first is by what
optimization level paramux is compiled with. If paramux is compiled with `Debug`
optimizations debug logs will be output to `stderr`. If paramux is compiled with
any other optimization the debug logs will not be output to `stderr`.

paramux also checks the `GHOSTTY_LOG` environment variable. It can be used
to control which destinations receive logs. paramux currently defines two
destinations:
