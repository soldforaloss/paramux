...
fn copySelectionToClipboards(
    ...
    if (copied and self.config.app_notifications.@"clipboard-copy") {
        try self.showAppNotification("paramux", "Copied selection to clipboard");
    }
}
...
pub fn performBindingAction(self: *Surface, action: input.Binding.Action) !bool {
    ...
    switch (action.scoped(.surface).?) {
        ...
        .clear_all_attention => return try self.rt_app.performAction(
            .{ .surface = self },
            .clear_all_attention,
            {},
        ),
        ...
        .toggle_window_on_top => return try self.rt_app.performAction(
            .{ .surface = self },
