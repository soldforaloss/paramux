...
param(
    ...
    [string]$Repo = "soldforaloss/paramux",
...
$primaryArtifactRootPath = if ($ArtifactRoot) {
    ...
    [System.IO.Path]::GetFullPath((Join-Path $repoRoot "dist/artifacts/paramux-$Version-windows-$primaryArch"))
}
...
$packageDescription = @"
paramux is a Windows terminal emulator that reuses Ghostty's terminal core under a native Win32 front end.
"@.Trim()
...
foreach ($arch in $Architectures) {
    $artifactRootPath = if ($ArtifactRoot) {
        ...
        [System.IO.Path]::GetFullPath((Join-Path $repoRoot "dist/artifacts/paramux-$Version-windows-$arch"))
    }
...
$metadata = [ordered]@{
    ...
    repository  = $Repo
    versioning  = [ordered]@{
        scheme = "major.minor follow the Ghostty upstream line; patch is the paramux release number on that line"
        line   = $versionLine
    }
    release     = [ordered]@{
