...
| [Features overview](https://ghostty.org/docs/features) | Accessibility is partial: the Win32 host exposes a UI Automation root provider (live window title) and the command palette exposes a list provider. Terminal scrollback is not yet served through `ITextProvider`, but the groundwork ships in `src/apprt/win32_uia/text.zig` (plain-text snapshot with pin map and UTF-16 offset table) so the COM wiring can stay thin. |
...
| Current distribution | The public `v0.1.16` release contains unsigned x64 and ARM64 portable ZIPs with per-arch `SHA256SUMS` files and the `install-paramux.cmd` PATH helper. Signed installer, WinGet, Scoop, and public stable channels are planned. |
