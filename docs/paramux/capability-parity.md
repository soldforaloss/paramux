...
| Native distribution | Partial | The local portable path guards its Paramux README, exact launcher VERSIONINFO, hooks, completion names, archive entries, checksum, and x64 baseline, and the live `v0.1.1` prerelease ships that guarded payload. The CLI manages the install lifecycle (`install`, `uninstall [--purge]`, checksum-verified in-place `update` from GitHub releases). Signed installer, public WinGet/Scoop, and verified ARM64 are not shipped. |
