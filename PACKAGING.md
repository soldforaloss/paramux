...
- Tags are `vMAJOR.MINOR.PATCH`, with no `-paramux.N` or other
  suffixes.
- The release title is the bare tag (for example `v0.1.11`) — no
...
- Artifact names keep their existing shape
  ...
  pipeline is unaffected; semver orders `0.1.11` above `0.1.0-paramux.9`.
...
- `paramux-0.1.11-windows-x64-portable.zip`
- `SHA256SUMS-windows-x64.txt`
- `paramux-0.1.11-windows-arm64-portable.zip` (native ARM64, built and smoke-tested on ARM64 CI runners)
...
```text
dist\artifacts\paramux-0.1.11-windows-x64\
```
...
Get-FileHash `
  .\dist\artifacts\paramux-0.1.11-windows-x64\paramux-0.1.11-windows-x64-portable.zip `
  -Algorithm SHA256
```
...
For `v0.1.11`, `NotSigned` is the expected signature status. A future
