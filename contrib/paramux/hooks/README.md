...
`winghostty +notify [--state=<state>] [--title=<t>] <message...>` emits an
...
Prerequisite: `winghostty` must be on `PATH` (Windows resolves `winghostty` to
`winghostty.com`, the console variant). Otherwise use the full path to
`winghostty.com` in the commands below.
...
```toml
notify = ["winghostty", "+notify", "--state=done", "Codex finished"]
```
...
1. **Wrapper alias** — run Gemini through a wrapper that emits
   `winghostty +notify --state=working ...` before launch and
   `winghostty +notify --state=done ...` after it exits.
...
`winghostty +notify --state=<state> ...` on its session-idle / session-complete
...
```
winghostty +notify --state=waiting testing paramux attention
```
